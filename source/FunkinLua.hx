	...
	public function new(script:String) {
		...
		try{
			...
			if(resultStr != null && result != 0) {
				trace('Error on lua script! error: ' + resultStr);
				#if (windows || android)
				lime.app.Application.current.window.alert(resultStr, 'Error on lua script! What happened?!');
				#else
		...
		#if ITG_MODCHARTS
		Lua_helper.add_callback(lua, "ITGMode", function(value:Bool) {
			ClientPrefs.notitg = value;
			#else
			luaTrace("ITGMode: ITG Mode is disabled in this build!", false, false, FlxColor.RED);
			#end
			return false;
		});	
